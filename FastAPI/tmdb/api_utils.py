...
key = 'fff26df164024d1778a1f35309e18f47'
...
class RequestApi:
    ...
    def get_artista_by_name(name):
        ...
        if data['results'] != []:
            ...
            r = requests.get(endpoint)
            data = r.json()
            results = {'id': data['id'], 'nome': data['name'], 'dataNascimento': data['birthday'],
                       'biografia': data['biography'], 'imagem': data['profile_path'], 'popularidade': data['popularity']}
            return results
